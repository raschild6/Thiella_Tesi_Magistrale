...
\begin{footnotesize}\textbf{Code in Scheme programming language notation:} ConceptNode example.\end{footnotesize}
\begin{python}
...
\begin{footnotesize}\textbf{Code in Scheme programming language notation:} InheritanceLink example, which specifies that a cat is an animal.\end{footnotesize}
\begin{python}
	InheritanceLink
		ConceptNode "cat"
		ConceptNode "animal"
...
Two interesting type of Atom that are derived from PredicateNode are the \textbf{DefinedPredicateNode} and the \textbf{GroundedPredicateNode}. \\
...
\begin{footnotesize}\textbf{Code in Scheme programming language notation:} EvaluationLink general structure, followed by a pratical example.\end{footnotesize}
\begin{python}
	EvaluationLink <tv>
...
There are other atom types used here. A more general list of Opencog atom types, which can be divided into categories, is this one:
...
\item Atom types for the OpenCog Probabilist Logic Network (PLN)
...
%%In this example Values are used by a PLN to perform probabilistic reasoning
%%To greatly simplify the work in this project, certain logic was used, so all TruthValues associated with atoms are always true, but, as amply demonstrated above, the system is set up to work in completely uncertain environments and with uncertain reasoning.
...
\cite{goertzel2008probabilistic}
